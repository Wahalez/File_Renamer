...
if __name__ == '__main__':
    ...
    if not os.path.exists(folder_path):
        print(Fore.RED + Style.BRIGHT + "Folder {0} doesn't exists".format(folder_path), file=stream)
        exit(1)
    ...
    temp = int(start_index) #temp index
    for file in os.listdir(folder_path):
        suffix = len(file) - file.find('.')
        print(Fore.CYAN + Style.BRIGHT + Back.BLUE + '{0:<25} TO {1:>20}'.format(file, new_name + str(temp) + file[-suffix:]), file=stream)
        temp += 1
    ...
    if answer in ['y','Y','yes','YES']:
        ...
        for file in os.listdir(folder_path):
            ...
            try:
                suffix = len(file) - file.find('.')
                os.renames(os.path.join(folder_path,file), os.path.join(folder_path , new_name + str(start_index) + file[-suffix:]))
